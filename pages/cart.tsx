...
function CartPage() {
  const { data } = useQuery<{ items: CartItem[] }, unknown, CartItem[]>(
    [`/api/get-cart`],
    () => fetch(`/api/get-cart`).then((res) => res.json()).then((data) => data.items)
  );
  ...
  const totalCost = useMemo(() => {
    ...
    return data.map((item) => item.price).reduce((prev, curr) => prev + curr, 0);
  }, [data]);
...
const Item = (props: CartItem) => {
  ...
  const [cost, setCost] = useState<number>(props.price);
  useEffect(() => {
    if (quantity != null) {
